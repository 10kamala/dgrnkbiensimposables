...
if(isset($_POST['update']))
{
    ...
    $status = 1; 
    $sql = "update tassujetti set FirstName=:fname,LastName=:lname,EmailId=:email,Gender=:gender,Dob=:dob,Address=:address,City=:city,Country=:country,Phonenumber=:mobileno,Status=:status where id=:did";
    ...
    $query->execute();
    $msg = "Assujetti est modifié avec succes";
}
?><!DOCTYPE html>
<html lang="en">
    <head>
        <!-- Title -->
        <title>Admin | Mise à jour d'un assujetti</title>
...
<?php include('includes/sidebar.php');?>
   <main class="mn-inner">
                <div class="row">
                    <div class="col s12">
                        <div class="page-title">Mise à jour d'un assujetti</div>
                    </div>
                    <div class="col s12 m12 l12">
                        <div class="card">
                            <div class="card-content">
                                <form id="example-form" method="post" name="updateas">
                                    <div>
                                                ...
                                                <div class="row">
                                                    <div class="col m6">
                                                        <div class="row">
                                                            <?php 
                                                            $did = intval($_GET['asid']);
                                                            $sql = "SELECT * from tassujetti where id=:did";
                                                            $query = $dbh->prepare($sql);
                                                            $query->bindParam(':did', $did, PDO::PARAM_STR);
                                                            $query->execute();
                                                            ...
                                                            <div class="input-field col  s12">
                                                                <label for="ascode">Code (Doit etre unique)</label>
                                                                <input  name="ascode" id="ascode" value="<?php echo htmlentities($result->AsId); ?>" type="text" autocomplete="off" readonly required>
                                                            <span id="asid-availability" style="font-size:12px;"></span> 
                                                            </div>
                                                            <div class="input-field col m6 s12">
                                                                <label for="firstName">Nom de l'assujetti</label>
                                                                <input id="firstName" name="firstName" value="<?php echo htmlentities($result->FirstName); ?>"  type="text" required>
                                                            </div>
                                                            <div class="input-field col m6 s12">
                                                                <label for="lastName">Postnom de l'assujetti </label>
                                                                <input id="lastName" name="lastName" value="<?php echo htmlentities($result->LastName); ?>" type="text" autocomplete="off" required>
                                                            </div>
                                                            <div class="input-field col s12">
                                                                <label for="email">Adresse mail de l'assujetti</label>
